...
class UserManager(models.Manager):
    ...
    def isValidRegistration(self, userInfo):
        ...
        if not userInfo['name'].replace(' ','').isalpha():
            registrationObject['errors'].append('Your name must contain alphabet characters only.')
            validRegistration = False
        if len(userInfo['user_name']) < 3:
            registrationObject['errors'].append('Username must be at least 3 characters.')
            validRegistration = False
        if User.objects.filter(user_name=userInfo['user_name']):
